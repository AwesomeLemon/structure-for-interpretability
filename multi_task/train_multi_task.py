...
def train_multi_task(param_file):
    ...
    log_dir_name = '/mnt/antares_raid/home/awesomelemon/runs3/{}_{}'.format(params['exp_id'],
    ...
    if len(log_dir_name) > 255:
        log_dir_name = '/mnt/raid/data/chebykin/runs3/{}'.format(datetime.datetime.now().strftime("%I_%M%p_on_%B_%d_%Y"))
    ...
    # train until convergence, then separate the next (i.e. previous) layer
    for stage in [2, 1, 0, -1]:  # range(len(filter_nums)):

        ignored_filters_per_task_per_layer = get_ignored_filters_per_task_per_layer(stage, filter_nums)
