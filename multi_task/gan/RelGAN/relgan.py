...
class Relgan():
    ...
    def __init__(self, args):
        ...
        self.path = '/mnt/raid/data/chebykin/celeb-hq-modified/celeba-hq/celeba-256'  # args.path
        self.lr = args.lr
        self.b1 = args.beta1
        self.b2 = args.beta2
        ...
        if self.if_continue:
            self.g_path = f"gans/model_18attr/generator107.h5"
            self.d_path = "gans/model_18attr/discriminator.h5"
    ...
    def get_model(self):
        ...
        self.d_model = Model(inputs=[self.imgA_input, self.imgB_input, self.vec_input_pos],
                             outputs=d_out)
        if self.if_continue:
            self.g_model.load_weights(self.g_path)
            self.d_model.load_weights(self.d_path)
    ...
    def get_imgs_tags(self, indexserX, imgIndex, imgAttr):
        ...
        for i in range(self.batch):
            ...
            img = io.imread(os.path.join(self.path, img_fa))  # str(temp_index)+".jpg"))
            imgs[i] = img / 127.5 - 1
    ...
    def train(self):
        ...
        imgAttr = np.load("anno_dic.npy", allow_pickle=True).item()
        print("training")
        ...
        def get_sample_data():
            indices = [1, 23401, 6056, 29635]#list(range(self.batch))
            imgs, atts = self.get_imgs_tags(indices, imgIndex, imgAttr)
            return imgs, atts
        ...
        for ep in range(int(self.epochs)):
            ...
            if ite % n_save_every == 0:
                ...
                if False:
                    ...
                    imgs, atts = get_sample_data()
                    generator_input = [imgs, 1-atts]
                    imgs_reversed_atts, _ = self.g_model.predict(generator_input)
                    width = self.batch #actually, width is height & vice versa
                    height = 2
                    images = np.concatenate([imgs, imgs_reversed_atts])
                ...
                filename = f"gans/img_{postfix}/fakeFace{ite // n_save_every}.jpg"
                new_im.save(filename)

                try:
                    self.g_model.save(f"gans/model_{postfix}/generator{ite // n_save_every}.h5")
                    self.d_model.save(f"gans/model_{postfix}/discriminator.h5")
                except:
                    print('Pass save')
            ite = ite + 1
