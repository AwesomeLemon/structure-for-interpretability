...
@click.option('--param_file', default='params/group_conv.json', help='JSON parameters file')
def train_multi_task(param_file, overwrite_lr=None, overwrite_lambda_reg=None, overwrite_weight_decay=None):
    ...
    def constrained_loss(criterion, output, target_var, U_regularizer_sum, std_conv_sum):
        loss = criterion(output, target_var)
        alpha = 0 if U_regularizer_sum.item() < std_conv_sum else -0.02
        loss_all = loss * torch.pow(std_conv_sum / U_regularizer_sum, alpha)
        return loss_all
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            loss = 0
            reps, u_reg_sum, complexity_sum = model['rep'](images)
            for i, t in enumerate(tasks):
                if if_train_default_resnet or True:
                    rep = reps
                else:
                    rep = reps[i]
                out_t, _ = model[t](rep, None)
                loss_t = constrained_loss(loss_fn[t], out_t, labels[t], u_reg_sum, complexity_sum)
        ...
        with torch.no_grad():
            for batch_val in val_loader:
                ...
                val_reps, u_reg_sum, complexity_sum  = model['rep'](val_images)
                for i, t in enumerate(tasks):
                    if True or if_train_default_resnet:
                        val_rep = val_reps
                    else:
                        val_rep = val_reps[i]
                    out_t_val, _ = model[t](val_rep, None)
                    loss_t = constrained_loss(loss_fn[t], out_t_val, labels_val[t], u_reg_sum, complexity_sum)
