...
class FaceAttributeFullConvDecoder(nn.Module):
    def __init__(self, task_connectivity):
        super(FaceAttributeFullConvDecoder, self).__init__()
        self.linear_conv = MaskedConv2d(512, 2, 1, task_connectivity)

    def forward(self, x, mask):
        x = self.linear_conv(x) #batch_size, 2, 1, 1
        out = F.log_softmax(x.squeeze(-1).squeeze(-1), dim=1)
        return out, mask