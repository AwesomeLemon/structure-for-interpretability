...
class MaskConResNet(nn.Module):
    ...
    def _create_connectivity_parameters(self, chunks_num, total_blocks_num, tasks_num=40):
        assert chunks_num == 8
        assert total_blocks_num == 2 * 3
        self.connectivities = []
        for i in range(total_blocks_num - 1):
            if self.if_fully_connected:
                cur_conn = torch.nn.Parameter(torch.ones((chunks_num, chunks_num), requires_grad=False).to(device))
            else:
        ...
        if False:
            if self.if_fully_connected:
                task_conn = torch.nn.Parameter(torch.ones((tasks_num, chunks_num), requires_grad=False).to(device))
            else:
    ...
    def forward(self, x, mask):
        ...
        out = self.conv1_2_ala_maxpool(F.relu(self.bn1(self.conv1(x)),inplace=True))
        # del x
        ...
        if_use_list_comprehension = True # if True, then faster, but uses more memory
        ...
        for i, layer in enumerate(self.layers):
            ...
            mask_binarized_cur = binarize(self.connectivities[i], self.K) if not self.if_fully_connected else self.connectivities[i]
            if if_use_list_comprehension:
                curs = torch.stack(curs, dim=1)
        ...
        if False:
            if if_use_list_comprehension:
                outs = [F.avg_pool2d(out, 16).view(out.size(0), -1) for out in outs]
            else:
                new_outs = []
                for out in outs:
                    new_outs.append(F.avg_pool2d(out, 16).view(out.size(0), -1))
                outs = new_outs
        else:
            outs = F.avg_pool2d(out, 8).view(out.size(0), -1)
