...
class CITYSCAPES(data.Dataset):
    ...
    def __init__(self, root, split=["train"], is_transform=False,
        ...
        self.DEPTH_MEAN = np.load('depth_mean.npy')[0]
    ...
    def __getitem__depth(self, index):
        '''
        this one is for calculating mean depth
        '''
    ...
    @staticmethod
    def calc_mean_depth():
        dst = CITYSCAPES(local_path, is_transform=True)
        bs = 1
        trainloader = DataLoader(dst, batch_size=bs, num_workers=0)
        total_samples = float(len(trainloader))
        print(total_samples)
        mean_depth = None
        for i, depth in enumerate(trainloader):
            if i % 20 == 0:
                print(i)
            if mean_depth is None:
                mean_depth = depth / total_samples
                print('none')
            else:
                mean_depth += depth / total_samples

        mean_depth_np = mean_depth.numpy()
        np.save('depth_mean.npy', mean_depth_np)
    ...
    def __getitem__(self, index):
        ...
        # depth[depth!=0] = depth[depth!=0] / self.DEPTH_STD
    ...
    def transform(self, img, lbl, ins_y, ins_x, depth):
        ...
        '''
        all rescaling is done to 256,512
        in the original code it was done to img.size//8, which is more readable, but using rescale is so nice...
        '''
        #todo: I don't think it's possible to have 256,512 output with 256,512 input with our architecture (first layers downsample, and there's no upsampling anywhere)
        # therefore I replace '.25' (i.e. 1/4) with 1/32
        lbl = skimage.transform.rescale(lbl, 1/32., multichannel=False, order=0, anti_aliasing=False)
        lbl = lbl.astype(int)
        # print(lbl.shape)

        ins_y = ins_y.astype(float)
        ins_y = skimage.transform.rescale(ins_y,  1/32., multichannel=False, order=0, anti_aliasing=False)

        ins_x = ins_x.astype(float)
        ins_x = skimage.transform.rescale(ins_x,  1/32., multichannel=False, order=0, anti_aliasing=False)

        depth = skimage.transform.rescale(depth,  1/32., multichannel=False, order=0, anti_aliasing=False)
...
if __name__ == '__main__':
    ...
    local_path = '/mnt/raid/data/chebykin/cityscapes/'
    # dst.calc_mean_depth()

    dst = CITYSCAPES(local_path, is_transform=True, augmentations=None)
    bs = 4
    ...
    for i, data in enumerate(trainloader):
        ...
        plt.savefig('cityscapes_example.jpg')
        plt.show()
        break