'''
Taken from https://github.com/CSAILVision/gandissect/blob/master/netdissect/broden.py
'''
...
class BrodenDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, directory='dataset/broden', resolution=384,
            ...
            broden_version=1, max_segment_depth=6, if_include_path=False):
        ...
        self.if_include_path = if_include_path
    ...
    def __getitem__(self, idx):
        ...
        #    'scene': [], 'texture': []}
        path = os.path.join(self.resdir, 'images', record['image'])
...
def test_broden_dataset():
    ...
    bds = BrodenDataset('/mnt/raid/data/chebykin/NetDissect-Lite/dataset', resolution=224,
            ...
            include_bincount=False, split='val', categories=["object", "part", "scene", "texture"])
    loader = torch.utils.data.DataLoader(bds, batch_size=100, num_workers=8)
    # for i in range(1,20):
    #     print(bds.labels[i]['name'],
    #             list(bds.category.keys())[bds.primary_category[i]])
    for i, (im, seg) in enumerate(loader):
        print(i, im.shape, seg.shape, seg.max())
